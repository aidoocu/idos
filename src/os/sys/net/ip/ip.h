...
#if defined(NRF24L01)
/* Esta interface no admite IP, tiene solo 32 bits, por lo que
utiliza VAMP por defecto */
#define VAMP_STACK
#endif /* NRF24L01 */
...
/* Si bien VAMP no tiene IP, o mas bien lo maneja de forma diferente,
   es necesario definir ...
   ToDo hay que ver que es necesario definir aqui y que no
   ...
   ... */
#ifdef VAMP_STACK
...
#define TCPIP_HLEN   0     /* No hay header TCP/UDP */
#define NET_BUFSIZE  32    /* Maximo tamaño de frame en VAMP */

uint16_t vamp_htons(uint16_t val);

#define ip_htons vamp_htons
